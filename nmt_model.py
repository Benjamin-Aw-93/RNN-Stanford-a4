...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        # Step 1 & 2
        self.encoder = nn.LSTM(self.model_embeddings.embed_size, self.hidden_size, bias = True, bidirectional = True)
        self.decoder = nn.LSTMCell(self.model_embeddings.embed_size + self.hidden_size, self.hidden_size, bias = True)
        # Step 3 & 4
        ...
        # Step 13, Vt is the size of the target vocabulary
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # Step 3
        init_decoder_hidden = self.h_projection.forward(torch.cat((last_hidden[0],last_hidden[1]), axis = 1))
        init_decoder_cell = self.c_projection.forward(torch.cat((last_cell[0],last_cell[1]), axis = 1))

        dec_init_state = (init_decoder_hidden, init_decoder_cell)

        ### END YOUR CODE
        # enc_hiddens is a packed sequence: (hidden layer values, lens)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1, dim=0):
            ...
            Ybar_t = torch.cat((Y_t, o_prev), dim = 1) # Same batch join Y_t tensor with o_prev
            dec_state, o_t, e_t = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
            combined_outputs.append(o_t)
            o_prev = o_t
        ### END YOUR CODE

        return torch.stack(combined_outputs, dim=0) # Stack [tensor,] on top of each other
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.squeeze(torch.bmm(enc_hiddens_proj, torch.unsqueeze(dec_hidden, dim=2)), dim = 2)# (b, src_len, h) (b, h , to add 1) => (b, src_len, 1)
